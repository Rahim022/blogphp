...
if ( isset($_POST['add_post']) ) {
    if (trim($_POST['title']) != "" && trim($_POST['author']) != "" && trim($_POST['category_id']) != "" && trim($_POST['body']) != "" && trim($_FILES['image']['name']) != "" ) {
        ...
        $post_insert = $conn->prepare("INSERT INTO posts (title, author, category_id, body, image,tags) VALUES (:title , :author , :category_id, :body, :image, :tags)");
        $post_insert->execute(['title' => $title, 'author' => $author, 'category_id' => $category_id, 'body' => $body, 'image' => $name_image, 'tags' => $tag]);
        ...
        <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
            ...
            <form method="post" class="mb-5" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="tags" placeholder=""  class="form-control" name="tags">
                    <small class="form-text text-muted">تگ های مقاله را وارد کنید.</small>
                    <span id="tag_name"></span>
                </div>
                <button type="submit" id="add_post" name="add_post" class="btn btn-outline-primary">ایجاد</button>
            </form>
